...
contract CountLimitedPrivileges is ERC721, IERC7766, IERC7766Metadata {
    ...
    mapping(uint256 tokenId => mapping(uint256 privilegeId => mapping(address to => bool))) privilegeStates;
    ...
    function exercisePrivilege(address _to, uint256 _tokenId, uint256 _privilegeId, bytes calldata _data) external {
        ...
        require(!privilegeStates[_tokenId][_privilegeId][_to], "Privilege for _to address has already been exercised");

        // Optional to deal with _data
        dealWithData(_data);

        ++privilegeCounts[_tokenId][_privilegeId];
        privilegeStates[_tokenId][_privilegeId][_to] = true;
        ...
        returns (bool _exercisable)
    {
        ...
        return privilegeCounts[_tokenId][_privilegeId] < MAX_COUNT && !privilegeStates[_tokenId][_privilegeId][_to];
    }
